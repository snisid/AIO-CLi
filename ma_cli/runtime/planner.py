...
class Plan:
    ...
    def get_next_step(self, completed: list[str]) -> PlanStep | None:
...
class Planner:
    ...
    STEP_TEMPLATES = {
        "create": [
            PlanStep(
                "analyze", "Analyze requirements and context", "architect", ["read_file", "search"]
            ),
            PlanStep("plan", "Create implementation plan", "architect", []),
            PlanStep(
                "implement", "Write code changes", "developer", ["write_file", "edit_file", "shell"]
            ),
        ...
        "modify": [
            PlanStep(
                "analyze", "Analyze current code and requirements", "architect", ["read_file"]
            ),
    ...
    def create_plan(self, intent: Intent, context: dict[str, Any] | None = None) -> Plan:
        """
