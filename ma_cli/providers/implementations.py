                ...
                if response.status_code == 200:
                    ...
                    for model_data in data.get("models", []):
                        model = DiscoveredModel(
                            ...
                            max_context_tokens=model_data.get("details", {}).get(
                                "context_length", 4096
                            ),
                            available=True,
                            raw_data=model_data,
...
class OmniRouteProvider(Provider):
    ...
    async def chat(self, messages: list[ChatMessage], model: str, **kwargs: Any) -> ChatResponse:
        ...
        async with httpx.AsyncClient(timeout=float(self._config.timeout)) as client:
            ...
            response = await client.post(
                f"{self._base_url}/chat/completions", json=payload, headers=self._get_headers()
            )
            response.raise_for_status()
    ...
    def is_installed(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["omniroute", "--version"], capture_output=True, text=True, timeout=5
            )
            return result.returncode == 0
        except Exception:
