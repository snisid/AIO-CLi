...
class CapabilityEngine:
    ...
    def infer(self, task_type: str, *, complexity: int = 3, risk: int = 1) -> TaskCapabilityRequest:
        ...
        level = self.select_reasoning_level(complexity, risk)
        return TaskCapabilityRequest(required=frozenset(required), reasoning=level)
    ...
    def can_satisfy(self, profile: CapabilityProfile, request: TaskCapabilityRequest) -> bool:
        if not profile.supports(request.required):
            return False
        if request.context_tokens and profile.max_context_tokens < request.context_tokens:
            return False
        supported = profile.supports_reasoning_levels
        if supported and self._reasoning_order[request.reasoning] > max(
            self._reasoning_order[level] for level in supported
        ):
            return False
        return True
