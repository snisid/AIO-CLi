...
class TestEventBus:
    ...
    def test_subscribe_and_publish(self):
        ...
        event = Event(
            event_type=EventType.TASK_CREATED, payload={"task_id": "test-123"}, source="test"
        )
    ...
    def test_event_history(self):
        ...
        for i in range(5):
            event = Event(event_type=EventType.TASK_CREATED, payload={"index": i}, source="test")
            asyncio.run(bus.publish(event))
    ...
    def test_emit_helper_function(self):
        ...
        event = emit_event(EventType.TASK_CREATED, payload={"test": True}, source="test_helper")
