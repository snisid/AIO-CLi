...
class ValidationEngine:
    ...
    def __init__(self, config: ValidationConfig | None = None, event_bus: EventBus | None = None):
    ...
    async def validate_task(
        ...
        if skipped_code_reviews:
            ...
            logger.warning(f"Task {task_id}: {len(skipped_code_reviews)} code review(s) skipped")
    ...
    def _determine_status(self, report: ValidationReport) -> str:
        ...
        if (
            not report.tests_passed
            or not report.code_review_passed
            or not report.security_review_passed
        ):
            return "FAILED"
...
class Finalizer:
    ...
    def __init__(self, validation_engine: ValidationEngine, event_bus: EventBus | None = None):
        self.validation_engine = validation_engine
        self.event_bus = event_bus
        self._finalized_tasks: set[str] = set()

    async def finalize_task(
        self, task_id: str, validation_report: ValidationReport | None = None
    ) -> tuple[bool, str]:
        """
        ...
        if not validation_report.can_finalize():
            ...
            logger.critical(f"FINALIZATION BLOCKED for task {task_id}: {reason}")
        ...
        if self.event_bus:
            await self.event_bus.emit(
                "finalized", {"task_id": task_id, "timestamp": datetime.utcnow().isoformat()}
            )
