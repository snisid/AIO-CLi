    ...
    def start_session(self, workspace_path: Optional[str] = None) -> str:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (session_id, AutonomyLevel.SUPERVISED_AUTO.value, workspace_path, now, now, "active"))
            conn.commit()
    ...
    def save_state(self, state: State) -> None:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                ...
                WHERE session_id = ?
            """, (state.autonomy_level.value, state.workspace_path, state.last_activity.isoformat(), session_id))
            ...
            for agent_id, status in state.agent_states.items():
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (session_id, agent_id, status.value, HealthStatus.HEALTHY.value, datetime.utcnow().isoformat()))
    ...
    def save_task(self, task: Task, session_id: Optional[str] = None) -> None:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                task.id, session_id, task.title, task.description, task.status.value,
                task.priority, task.assigned_agent, task.assigned_role,
                task.created_at.isoformat() if task.created_at else None,
                task.started_at.isoformat() if task.started_at else None,
                task.completed_at.isoformat() if task.completed_at else None,
                task.result, task.error,
                json.dumps(task.outputs), json.dumps(task.metadata),
    ...
    def record_event(self, event: Event, session_id: Optional[str] = None) -> None:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (
                session_id, event.event_type.value,
