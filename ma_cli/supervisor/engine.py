...
class Supervisor:
    ...
    def get_system_health(self) -> SystemHealth:
        ...
        health = SystemHealth(
            ...
            queued_tasks=sum(1 for p in self._processes.values() if p.status == ProcessStatus.QUEUED),
    ...
    def cleanup_completed(self, older_than_seconds: int = 3600) -> int:
        ...
        for process_id, process in list(self._processes.items()):
            if process.status in (ProcessStatus.COMPLETED, ProcessStatus.FAILED, ProcessStatus.CANCELLED, ProcessStatus.TIMEOUT):
