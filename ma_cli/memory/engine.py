...
class SQLiteMemoryBackend(MemoryBackend):
    ...
    def search(self, query: str, filters: dict[str, Any] | None = None) -> list[MemoryEntry]:
        ...
        if filters:
            if "memory_type" in filters:
                ...
                params.append(filters["memory_type"].value if isinstance(filters["memory_type"], MemoryType) else filters["memory_type"])
            
            if "scope" in filters:
                conditions.append("scope = ?")
                params.append(filters["scope"].value if isinstance(filters["scope"], MemoryScope) else filters["scope"])
    ...
    def get_summary(self, filters: dict[str, Any] | None = None) -> MemorySummary:
        ...
        with self._get_connection() as conn:
            ...
            # Storage size
            cursor.execute(f"SELECT SUM(length(content)) FROM memory_entries {where_clause}", params)
            storage_size = cursor.fetchone()[0] or 0
            
            # Oldest and newest
            cursor.execute(f"SELECT MIN(created_at), MAX(created_at) FROM memory_entries {where_clause}", params)
            row = cursor.fetchone()
...
class MemoryEngine:
    ...
    def get_conversation(
        ...
        return [
            ConversationMessage(
                content=e.content,
                role=e.metadata.get("role", "assistant"),
...
class SessionManager:
    ...
    def create_session(
        ...
        # Generate a project_id if not provided
    ...
    def load_session(self, session_id: str) -> SessionState | None:
        ...
        state = SessionState(
            ...
            last_activity=datetime.fromisoformat(metadata.get("last_activity", entry.updated_at.isoformat())),
